...
- **Regenerate** [doctrine/SITEMAP.md](doctrine/SITEMAP.md) when you add, remove, or rename Markdown under `doctrine/`: run `./scripts/generate-doctrine-sitemap.sh` (POSIX) **or** `python scripts/generate_doctrine_sitemap.py` (any platform, including Windows) from the repo root. Both produce identical output.
...
- **Install validation dependencies:** use Python 3.10+ and `python -m pip install -r requirements.txt`. CI currently uses Python 3.12; the requirements declare compatible ranges, not an exact lockfile.
- **Validate contracts** if you touch anything under `contracts/`: run `python scripts/validate-contracts-v1.py`. CI runs this on every push/PR ([.github/workflows/contracts.yml](.github/workflows/contracts.yml)).
- **Check local links:** `python scripts/check_markdown_links.py` checks Git-tracked Markdown, including newly staged files, from any working directory. It validates local link/image targets and `ENGINEERING.md` fragments, not external URLs or fragments in other files. Diagnostics use the containing Markdown block's starting line. Pass repository-relative paths to check untracked drafts explicitly. Regression suite: `python -m unittest discover -s scripts/tests -p "test_*.py" -v`.
- **Validate printed YAML:** `python scripts/check_doctrine_examples.py` checks YAML fences in tracked doctrine Markdown. Put a `doctrine-example` JSON HTML comment immediately before each fence: `<!-- doctrine-example: {"schema":"contracts/run-contract.v1.schema.json","wrapper":"run_contract"} -->` for a complete owned example, or `{"partial":"reason"}` / `{"external":"reason"}` inside the same comment form for a syntax-only excerpt or externally owned format. Omit `wrapper` for an unwrapped schema instance. Reasons must state the boundary; an excerpt is not a complete-contract claim. The check parses and validates data, never executes examples or proves runtime behaviour.
