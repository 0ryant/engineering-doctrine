...
- **Agent financial authority (G11)** — [cost-and-finops.md](doctrine/principles/cost-and-finops.md) new **§7.1**: **default-deny** payment/transaction authority (per-agent, inventory-recorded grants on dedicated §2.1 principals), session/run transaction budgets enforced outside the prompt, **materiality-tiered human approval** (irreversible/critical starts at approve-before-execute), attribution-chain payment audit trails with retry idempotency; AP2/Stripe/Mastercard/Visa standards watch (informative); agentic-loop §6 gains **money-movement** in the approve-before-irreversible enumeration; ASI02/ASI03 crosswalk links; glossary entry. [ADR 0035](docs/adr/0035-add-agent-financial-authority-adaptive-evaluation-and-spend-governance.md).
- **Adaptive defence evaluation (S3)** — [testing-strategy.md](doctrine/principles/testing-strategy.md) §5: agent tool-call paths + agent/skill configuration join the tiered adversarial surfaces; injection/hijack defence claims at blocking tiers MUST be evidenced by **adaptive (attacker-iterates) evaluation** with explicit trigger (deployment / material change / model swap, not per merge) and **minimum viable form** (AISI-protocol human red-team, ~25 attempts); [agentic-loop-design.md](doctrine/patterns/agentic-loop-design.md) new **§9.4** — adopting the dual-path/CaMeL architecture does not discharge the testing duty (its published validations are static-benchmark only); ai-adoption-controls §3 agent-hijack row + taxonomy refresh.

### Changed

- **Vendor-native spend governance (S4)** — [cost-and-finops.md](doctrine/principles/cost-and-finops.md) §7/§4: prefer platform spend limits/entitlements (Azure agent units, Bedrock profiles/quotas, OpenAI/Anthropic org limits) with self-built breakers as the **backstop layer and portable floor that remains in place**; agent-unit metering feeds anomaly scans; Tier-D capacity planning. Additive guidance.
- **SP 800-53 Release 5.2.0 (S5)** — citations updated in [REFERENCES](doctrine/REFERENCES.md), [audit-logging.md](doctrine/principles/audit-logging.md), and [normative-language-applicability-and-exceptions.md](doctrine/patterns/normative-language-applicability-and-exceptions.md); [engineering-controls-governance-program.md](doctrine/patterns/engineering-controls-governance-program.md) cadence table records the new controls (SI-02(07), SA-15(13), SA-24) as next-baseline-diff candidates. Editorial.
