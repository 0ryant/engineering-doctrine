...
The operator supplied *AI-Native Software Authority Lifecycle — Council Session 1*, a non-binding proposal that recasts AI-native delivery as a governed state-transition system. Its core separation of intended state, executable change, authority, verification evidence, and observed production reality is compatible with this library, but the proposal cannot be adopted unchanged:
...
- it proposes a universal transition schema before a real brownfield pilot has tested the minimum record and blocked paths.
...
6. Adopt brownfield stages P0–P5. Pilot one real bounded change, including at least one failed or inconclusive path, before standardising a repository-wide transition schema.
...
### Adopt The Council Proposal Unchanged
...
Rejected for initial adoption. It risks automating an untested abstraction and duplicating issue, repository, CI/CD, artefact, and observability systems. A pilot will establish which fields need a portable schema.
...
- Transition-record schema, retention profile, and empirical autonomy thresholds remain deferred until pilot evidence exists.
...
- A transition schema created before the pilot could freeze local assumptions; no schema is adopted by this ADR.
