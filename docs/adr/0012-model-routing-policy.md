...
> Promoted `proposed → accepted` 2026-05-20 per operator decision packet (Decision B). Council vote 4/4 PROMOTE; empirical anchor is the 9-cell Stage 2 scoreboard + 4 governance tools at N≥100 hard-enforcement + narrow-scope model-PRIMED narrow-scope decision. Promotion does NOT ship runtime enforcement — composer v0.2 wires the policy. See `evidence-workbook/18-cross-product-test/v3/operator-decisions-2026-05-20.md`.

> Note on numbering. The v3 position update at
> [`evidence-workbook/18-cross-product-test/v3/v3-position-update.md`](https://github.com/0ryant/evidence-workbook)
...
> [`evidence-workbook/18-cross-product-test/v3/v3-FINAL-SYNTHESIS.md`](https://github.com/0ryant/evidence-workbook)
> §6 ship-list (item 4) points at this file.

## Context

The v2 cross-product test ([`evidence-workbook/18-cross-product-test/v2/`](https://github.com/0ryant/evidence-workbook)) and the v3 matched-pair priming addendum produced a 9-cell empirical scoreboard that destroys the naive "bigger model is better" routing rule. The numbers, all from blind canonical re-scoring (not self-scores), are reproduced verbatim from [`v3-FINAL-SYNTHESIS.md`](https://github.com/0ryant/evidence-workbook) §2.1:

| Cell | Canonical | $ per cell | Δ self-canonical |
|------|----------:|-----------:|-----------------:|
| tools-model-high-capability + primer | 87 | ~$3 | 0 |
| **tools-model-balanced + primer (production default)** | **85** | **$0.58** | **−1** |
| native-model-high-capability + primer | 78 | ~$2-3 | −2.5 |
| tools-model-balanced (no canonical primer) | 78 | $0.58 | −11 |
| **tools-narrow-scope model + primer (narrow-scope tier)** | **73** | **~$0.21** | **+10** |
| native-model-balanced | 70 | $0.38 | −21 |
| tools-narrow-scope model-no-skills | 61.5 | $0.21 | −16.5 |
| native-narrow-scope model | 47.5 | $0.17 | −35.5 |
| tools-narrow-scope model + SKILLS (DEPRECATED) | 31 | $0.20 | −65 |
...
1. **The 3-tier shape is empirical, not aesthetic.** Three cells sit on the production frontier: model-high-capability-primed at canonical 87, model-balanced-primed at canonical 85, narrow-scope model-primed at canonical 73. model-balanced-primed beats native-model-high-capability by 7 points at 20% of the cost and lands within 2 points of model-high-capability-primed at ~19% of its cost. The 200-token primer text artifact (skill `anti-confab-200tok@1.0.0`, hash `c138dd966c82f7bd792684ab3fef0f50d75aa9342468db8b5d265f24f3fb35a8` per council D5) closes most of the gap an model-high-capability tier would.

2. **The primer is model-conditional.** Confirmed-uplift on model-high-capability and model-balanced, **inverted** on narrow-scope model when bundled with full SKILLS — the `tools-narrow-scope model + SKILLS` cell (canonical 31, self-scored 96) is the silent-stub class. The narrow-scope narrow-scope model-PRIMED tier at canonical 73 is the production-ready *minimal* narrow-scope model surface, gated behind mandatory external review because Δ self-canonical is **+10** (the cell over-claims by 10 points and cannot grade its own work).

3. **A new host-policy-layer failure mode landed in the same evidence run.** The `bash -c '<benign>'` interpreter-wrapper class passes every host-policy-layer hook guard: the first-token check sees `bash` not `sudo/su/doas`; the protected-paths guard does not run on bash's argv; the privilege-escalation detector has no interpreter-wrapper rule. CC-2 is FALSIFIED. The host-policy-layer-side fix has since landed (per the v3 per-tool failure-mode tests), but the router still refuses the pattern structurally as defense-in-depth.
...
The commercial framing matters. Anyone can publish a model id. Nobody can publish, without the underlying cohort, the empirically-grounded refusal rules — "we refuse narrow-scope model for greenfield because we measured a 73 canonical with Δ +10 self-score on this exact class of task" is the kind of statement competitors cannot reproduce by reading our marketing. The router policy IS the product surface where the v2 negative-result cohort and the v3 9-cell scoreboard become a runtime decision. The policy is the moat the doctrine secures.

Inputs that informed this decision:

- [`evidence-workbook/18-cross-product-test/v3/v3-FINAL-SYNTHESIS.md`](https://github.com/0ryant/evidence-workbook) §2.1 9-cell scoreboard and §6 council-D4 resolution.
- [`evidence-workbook/18-cross-product-test/v3/v3-position-update.md`](https://github.com/0ryant/evidence-workbook) — §6 sketch and §11 evidence addendum.
- [`evidence-workbook/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring.md`](https://github.com/0ryant/evidence-workbook) — the original 5-cell blind re-scoring (canonical 31 / canonical 78 / canonical 70).
- [`evidence-workbook/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring-model-balanced-primed.md`](https://github.com/0ryant/evidence-workbook) and [`canonical-scoring-model-high-capability.md`](https://github.com/0ryant/evidence-workbook) — primed-addendum cells.
- [`evidence-workbook/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring-narrow-scope model-primed.md`](https://github.com/0ryant/evidence-workbook) — the narrow-scope tier evidence (canonical 73, Δ +10).
- [`evidence-workbook/18-cross-product-test/v3/results/plan-b-v2-claim-audit-against-stage2/composite.md`](https://github.com/0ryant/evidence-workbook) — claim_audit v2 catch-rate evidence backing the FALSIFIED-escalation rule.
- [`evidence-workbook/18-cross-product-test/v2/results/per-tool-failure-mode-tests-results/composite.md`](https://github.com/0ryant/evidence-workbook) — the CC-2 falsification.
...
Adopt a v1 **router policy** contract that the host-policy-layer-hosted router consumes to (a) resolve every cell to one of three tiers, (b) escalate on adversarial-verifier verdicts, and (c) refuse routing for empirically-known failure modes. The contract surface is:
...
2. **Worked examples:**
   ...
   - [`contracts/examples/enterprise-strict.router-policy.yaml`](../../contracts/examples/enterprise-strict.router-policy.yaml) — escalates immediately on the first FALSIFIED, refuses narrow-scope model for everything beyond `scaffolded_typed_authority`, lowers per-session kill.
   - [`contracts/examples/experimental-narrow-scope model-narrow-scope.router-policy.yaml`](../../contracts/examples/experimental-narrow-scope model-narrow-scope.router-policy.yaml) — the narrow-scope model-PRIMED narrow-scope tier exercised in production with mandatory external review.
...
| Refusal | `narrow-scope model-no-artifact-production-without-primer` | family == model_narrow AND cell_class == artifact_production AND primer == null | Refuse outright; cite v2 canonical-31 cell |
| Refusal | `narrow-scope model-no-greenfield` | family == model_narrow AND cell_class == greenfield_backbone | Refuse outright; cite Δ +10 self-canonical on narrow-scope model-PRIMED |
| Refusal | `narrow-scope model-narrow-scope-requires-external-review` | family == model_narrow AND cell_class == scaffolded_typed_authority | Mandate non-self scoring (not a hard refusal — the tier is permitted with the review) |
...
| Default tier is `tools-model-balanced-primed`, not `tools-model-high-capability-primed` | model-balanced-primed canonical 85 beats native-model-high-capability 78 by 7 points at 20% of the cost, and lands within 2 of tools-model-high-capability + priming at ~19% of its cost. The cost gradient is steep enough that defaulting to model-high-capability would burn 5× the budget for a 2-point lift. | v3 position update §1; canonical-scoring-model-balanced-primed.md. |
| Premium tier reserved for {security_critical, supervisor_change, doctrine_promotion} | Cost-of-wrong-answer dominates cost-of-tier for these three. Supervisor changes alter the gate enforcing every other run; doctrine promotion edits the policy the router itself loads next session. A wrong call at model-balanced tier compounds. | v3 §6, §9 subtask 7; ADR-0006 governance ADR. |
| Narrow_scope tier permitted ONLY for `scaffolded_typed_authority`, never for greenfield | narrow-scope model-PRIMED hit canonical 73, but Δ self-canonical is +10. On scaffolded subtasks (wire format + tests already in place from a higher tier) the cell has structural rails. On greenfield it shipped the canonical-31 silent stub. The scoping difference is the load-bearing reason narrow-scope model stays in the schema at all. | canonical-scoring-narrow-scope model-primed.md; v3 §3 narrow-scope note. |
| Narrow_scope MUST have `external_review_required: true` with sample rate 1.0 | Δ +10 means the cell cannot grade its own work. v1 says every run is sampled; the rate is only lowerable when independently measured. | canonical-scoring-narrow-scope model-primed.md self-Δ rows; council D4 4/4. |
...
| Refusal rules require an `evidence_ref` field, schema-mandated | Without it, "we refuse narrow-scope model" is a vibes-claim. The commercial moat needs auditable provenance per rule. | Schema `$defs.refusal_rule.required: [rule_id, when, action, evidence_ref]`. |
...
| narrow-scope model refused for `greenfield_backbone` specifically (not all classes) | The empirical cell that fired is `tools-narrow-scope model + SKILLS` canonical 31 on a greenfield-style binary. Refusing all narrow-scope model would lose the legitimate narrow-scope tier (canonical 73 at $0.21). The refusal is scoped to the class where the silent stub fired. | canonical-scoring.md §3; v3 §3 narrow-scope note. |
| narrow-scope model refused on `artifact_production` only when primer is null | Distinct from the unconditional `greenfield_backbone` refusal. Build-class cells without the primer are the v2 catalog `inverted_avoid` shape; the primed narrow_scope tier handles bounded artifact_production via the require_tier escalation. | doctrine/skills/anti-confabulation.skill.md model_compatibility; canonical-scoring.md. |
| `primer-required-on-build-class` enforces canonical primer on every build/artifact/refactor cell | Tools-model-balanced without the primer scored canonical 78 with Δ self-canonical −11. With the primer it scored canonical 85 with Δ −1. The +14 self-canonical swing IS the empirical commitment of the priming pattern; permitting unprimed build cells contaminates measurement and lift. | canonical-scoring.md (unprimed model-balanced vs primed model-balanced rows). |
| Interpreter-wrapper command pattern refused at the router layer, not deferred to host-policy-layer | CC-2 is FALSIFIED on host-policy-layer's side; the fix has since landed but the v2/v3 corpus retains the failure as evidence. Refusing at the router means the refusal is visible to every consumer of the policy, not just host-policy-layer-aware ones. Defense-in-depth at the routing layer. | composite.md CC-2 FALSIFIED rows; v3 §2.4. |
...
| Per-cell caps keyed by tier name AND replicated at tier level (`max_cost_usd_per_cell`) | Two layers because the tier-level cap is the empirical floor (an model-high-capability cell costs ~$3, so anything <$3 strands the tier) while the per-cell-ceilings table is the operator's session-wide cost discipline. The router takes the LOWER of the two. | Cost-gradient table v3-FINAL-SYNTHESIS §2.1. |
...
- The commercial-moat claim becomes concrete. "We refuse narrow-scope model for greenfield" stops being a slide and becomes a `refuse_routing: true` line with an `evidence_ref` pointer at the v2 canonical-scoring file.
- Operators can ship variant policies (enterprise-strict, experimental-narrow-scope model-narrow-scope) without forking the schema. The three shipped examples are the canonical demonstrations of how variation looks; new estates copy one and edit numbers.
...
- The narrow_scope tier is permitted in production with mandatory external review, preserving the $0.21/cell economics of narrow-scope model for scaffolded subtasks without exposing the +10 Δ self-canonical risk to operator review.
...
- narrow-scope model-no-primer routing on `build_class`, `artifact_production`, or `refactor_class` is rejected by the `primer-required-on-build-class` refusal rule across all shipped examples. Closing the "agent strips the primer and ships anyway" path.

### Open work

- The narrow-scope narrow-scope model tier ships with `external_review_sample_rate: 1.0`. Lowering the rate requires an independently-measured non-self canonical run on a fresh adversarial cohort and operator sign-off.
- The `host-policy-layer-hook-bypass-fix` has landed at host-policy-layer v2; the router-layer `interpreter-wrapper-block` refusal remains as defense-in-depth and an evidence anchor. Removing it from the router would require a separate ADR (it is not just a "now host-policy-layer handles it" decision — the router-layer refusal is visible to non-host-policy-layer consumers).
...
- A v3.5 ship-list item — runtime enforcement of `require_external_review: true` — is not in scope for this ADR. The schema declares the field; the in-tree host-policy-layer implementation lands separately.
...
- Cross-link from [`evidence-workbook/18-cross-product-test/v3/v3-FINAL-SYNTHESIS.md`](https://github.com/0ryant/evidence-workbook) §6 ship-list item 4 points at this file.
...
A second residual: the narrow_scope tier's `external_review_sample_rate: 1.0` is a v1 conservative position. The operator may want to lower it once independent measurement is on the record, but until then every narrow-scope model-PRIMED narrow-scope run is gated by a non-self reviewer. This is loud-not-silent; the schema does not let the rate drop below 0 (`minimum: 0`) and does not let it silently disappear (it is `required` whenever `external_review_required: true`).

## Evidence

- Empirical anchor table in §Context above, traced row-by-row to canonical-scoring.md, canonical-scoring-model-balanced-primed.md, canonical-scoring-model-high-capability.md, canonical-scoring-narrow-scope model-primed.md.
