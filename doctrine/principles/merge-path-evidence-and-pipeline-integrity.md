...
| 2 | Field incidents show **CI/CD graphs** and **privileged tokens** are abused as often as application bugs; NIST SSDF **PS** explicitly addresses protecting development pipelines. **Behaviour-as-code** (agent-definition artefacts) now directs the same privilege: a wiper instruction shipped as agent prompt text — not executable code — inside an official Amazon Q Developer VS Code release (https://www.csoonline.com/article/4027963/hacker-inserts-destructive-code-in-amazon-q-as-update-goes-live.html; vendor bulletin https://aws.amazon.com/security/security-bulletins/AWS-2025-019), and a hallucinated package spread to 237+ repos through unreviewed AI-generated skill files (https://www.aikido.dev/blog/agent-skills-spreading-hallucinated-npx-commands). |
