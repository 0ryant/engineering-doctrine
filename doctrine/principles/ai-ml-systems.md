...
- **Multi-agent “council”** outcomes treated as **approval** without **CI** or **human** sign-off for high-risk change.- **Self-certifying output verification**: the agent evaluates its own output with no external signal (test run, structured rubric, human check). Without an external verifier, the loop is open; confident wrong answers are worse than uncertain right ones.
