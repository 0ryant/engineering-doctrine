...
- **Loop economics**: in iterative agent loops the cost driver is **per-iteration context growth** — each pass re-sends the accumulated context, so billed input grows roughly quadratically unless the prompt prefix is cache-stable (cached reads run to roughly an order of magnitude cheaper than uncached input, with ratios varying and drifting by vendor — dated specifics in the ledger; any edit to earlier context silently re-bills the whole prefix) and compaction is applied, noting compaction is itself a billed, lossy pass. Track **generated-to-accepted ratio** and **cost per accepted result** ([../patterns/ai-native-software-development-lifecycle.md](../patterns/ai-native-software-development-lifecycle.md) §16) as **near-horizon monitoring and stop signals, never KPIs to maximise** — acceptance-style metrics are gameable, and long-horizon survival is confounded by stewardship rather than quality (single-source finding, ledger L6.3). The loop-vs-manual break-even is a **formula with estate-set parameters** (keep rate × per-iteration cost against the manual alternative), never a portable threshold; published evidence shows net-negative loops occur while operators self-estimate the opposite, so self-perception is not a stop signal. Ceiling enforcement lives in [../patterns/agentic-loop-design.md](../patterns/agentic-loop-design.md) §1.2, §6.1, and §8.3. Graded sources: [../evolution/research-agent-loop-graph-cross-check-2026-08.md](../evolution/research-agent-loop-graph-cross-check-2026-08.md) §5 and second-pass addendum.
