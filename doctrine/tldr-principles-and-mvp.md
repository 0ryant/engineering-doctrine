...
| 9 | **AI-assisted change** — repo stays **SoR**; trace objective → guardrailed outcome measures → intervention → bounded task → output → observed outcome; never equate output with value; tiers A–D and materiality are separate; every agent execution is bounded by a run contract; **same** CI/review bar; authority and enactment remain explicit. | [`ai-ml-systems.md`](principles/ai-ml-systems.md) §§2–8, [`patterns/ai-native-software-development-lifecycle.md`](patterns/ai-native-software-development-lifecycle.md), [`patterns/run-contracts.md`](patterns/run-contracts.md), [`tooling/ai-assisted-development.md`](tooling/ai-assisted-development.md), [`patterns/rag-retrieval-baseline.md`](patterns/rag-retrieval-baseline.md) |
