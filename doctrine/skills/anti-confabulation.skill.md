...
> tightening) on Sonnet 4.6, holding model and backbone fixed. Empirical
> basis: `value-sheet/18-cross-product-test/v2/results/test-1-backbone/`.
...
- **Sonnet 4.6 / 4.7:** REQUIRED for any build-class contract. Empirical lift
  +7 canonical points / 12-point delta tightening
  (`canonical-scoring-sonnet-primed.md:238-251`).
- **Opus 4.7:** OPTIONAL but recommended. Empirical delta floor 0 to −2.5
  (`canonical-scoring-opus.md:50-58`). The Opus lift is small but the
  intervention is cheap.
- **Haiku 4.7:** EXPERIMENTAL. Unmeasured with priming in Stage 2; the
  unprimed Tools-Haiku cell over-claimed by 65 points
...
run_contract:
  ...
  context:
    skills:
      - pr-evidence-mcp        # build-class skill; triggers auto-bundle
...
- The assertion is `inconclusive` if the executing engine
  (`tapprove.claim_audit`) has not yet shipped the `priming_active` kind
  implementation.
...
- **Treating priming as a substitute for the verifier pack.** Priming
  ...
  are mandatory.
- **Applying priming to Haiku without measurement.** The `required_for`
  semantics mark Haiku experimental for a reason — priming may be net-neutral
  or net-harmful on a model without sufficient meta-cognitive capacity. Run
  per-model measurement before promoting Haiku from `experimental_for` to
  `required_for`.
...
- `value-sheet/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring.md`
  — 5-cell unprimed baseline; Tools-Haiku silent-stub at lines 39-43.
- `value-sheet/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring-opus.md`
  — 2-cell Opus pass with priming; Tools-Opus delta 0 at line 52;
  Native-Opus delta −2.5 at line 53.
- `value-sheet/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring-sonnet-primed.md`
