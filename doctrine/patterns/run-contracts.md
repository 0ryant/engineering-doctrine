...
- **Workflow DAG composition.** Multi-step orchestration ("contract A's output is contract B's input") is out of scope. Until a portable workflow primitive exists, an external coordinator owns the lineage and coordination record without mutating either contract. v2 may introduce a `dependencies: [<fingerprint>]` field or a separate workflow primitive that wraps contracts. **Deferral record (ADR 0039, per [doctrine-content-lifecycle.md](doctrine-content-lifecycle.md) §8):** owner — the operator; review trigger — the first genuine multi-contract composition need in the estate, or a lifecycle sweep, whichever first; retirement condition — the debt retires when a v2 `dependencies`/workflow primitive ships, or when an ADR records the need extinct. The field name itself is to be reconsidered against the package-dependency sense of "dependencies" (e.g. `edges`, `needs`, `upstream`) when the v2 work runs. The design-ingredient record (node-status vocabulary as schema prerequisite; the shape menu with its lossless list→expression upgrade; per-consumer failure-policy declaration; fan-out bounding; converge declaration; re-run scope) is held in [../evolution/research-agent-loop-graph-cross-check-2026-08.md](../evolution/research-agent-loop-graph-cross-check-2026-08.md) §5 second-pass addendum (rows G6.1–G6.6 and its §4 row 2 amendment) so the future v2 work does not re-research.
