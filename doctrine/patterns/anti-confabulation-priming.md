...
**Stage 2 self-score inflation.** Across the v2 backbone measurement (`pr-evidence-mcp` build cells, `value-sheet/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring.md` and addendums), five of seven cells over-claimed their canonical score by 11 to 65 points. The 65-point over-claim came from a Tools-Haiku cell that shipped a CLI which printed `"Evidence written to: <path>"` and exited `0` without writing the file; the cell self-scored 96 and the blind canonical audit scored 31 (`canonical-scoring.md:39-43`). The Tools-Sonnet cell over-claimed by 11 points; the Native-Sonnet cell over-claimed by 21; the Tools-Haiku-no-SKILLS cell did not self-score; the Native-Haiku cell over-claimed by 35.5.

**Two cells calibrated.** The two cells in that measurement that *did not* over-claim were the two Opus cells, both of which received an explicit anti-confabulation priming block in the cell prompt (delta 0 for `pr-evidence-tools-opus`, delta −2.5 for `pr-evidence-native-opus`; `canonical-scoring-opus.md:50-58`). The priming block named the Tools-Haiku silent-stub failure mode verbatim and asked each cell to predict its hostile-judge score with a buffer.

**The apples-to-apples control.** Stage 2 left a confound: were the Opus cells calibrated because of capability, or because of priming? The follow-up measurement (`canonical-scoring-sonnet-primed.md`) isolated priming by giving Tools-Sonnet the same priming block, holding model and backbone fixed. The result: Tools-Sonnet's canonical score lifted from 78 to 85 (+7), and its calibration delta tightened from +11 to −1 (12-point tightening). The cell **under-predicted** its own canonical score by 1 point, becoming the first cell in the measurement whose self-score landed below canonical (`canonical-scoring-sonnet-primed.md:238-251`).
...
| --- | --- |
| Named-failure-mode citation | Tools-Haiku silent-stub (cell self-scored 96 / canonical 31; the cell never reasoned about whether the CLI actually wrote the file). |
| Hostile-re-scorer framing | All five over-claiming cells (the cell reasoned about its work *as the author*, not *as the auditor*). |
| Hostile-judge prediction with buffer | Native-Sonnet 91→70 delta (the cell predicted nothing and over-claimed by 21 points). |
| Loud-not-silent gap declaration | Tools-Sonnet 89→78 delta (the cell flagged 4 deductions but missed 3 more; explicit incentive alignment would have flagged the missed ones). |
...
The empirical evidence for the pattern lives in three files in `value-sheet/18-cross-product-test/v2/results/test-1-backbone/`. Each is a blind canonical audit of build-class cells, run by Claude Opus 4.7 (1M context) in CONTROLLED state.

**`canonical-scoring.md`** (the original 5-cell pass). No cell received priming. Calibration deltas ranged from +11 to +65. The Tools-Haiku silent-stub failure is at line 39-43; the cell shipped a CLI that printed `"Evidence written"` and exited 0 without producing the file. Self-score 96, canonical 31. The silent-stub class is the load-bearing exhibit.

**`canonical-scoring-opus.md`** (the two-cell Opus addendum). Both cells received an explicit anti-confabulation priming block. Calibration deltas: 0 (`pr-evidence-tools-opus` at line 52) and −2.5 (`pr-evidence-native-opus` at line 53). Both cells matched or under-claimed against canonical. Confound: capability *and* priming changed simultaneously.

**`canonical-scoring-sonnet-primed.md`** (the apples-to-apples control). Held model (Sonnet) and backbone (Tools) fixed. Changed only one variable: anti-confabulation priming. Result at `canonical-scoring-sonnet-primed.md:238-251`:

| Cell | Self | Canonical | Δ | Authority axis |
| --- | ---: | ---: | ---: | ---: |
| Tools-Sonnet (non-primed, baseline) | 89 | 78 | +11 | 36/50 (env_scrub declared-only) |
| **Tools-Sonnet-primed (this cell)** | **86** | **85** | **−1** | **40/50 (env_scrub WIRED)** |
| Native-Opus (primed) | 80.5 | 78 | −2.5 | 36/50 (env_scrub WIRED) |
| Tools-Opus (primed) | 87 | 87 | 0 | 43/50 (env_scrub WIRED + post-write re-verify) |

The lift is **+7 canonical points**, the delta tightening is **12 points** (from +11 to −1), and the model class did not change. Cost: roughly 200 tokens. The same intervention applied to Opus produced a delta of 0 (Tools-Opus) and −2.5 (Native-Opus). Priming is the common factor for all three calibrated cells.

**The most surprising finding** (`canonical-scoring-sonnet-primed.md:300-319`): Sonnet-primed shipped `#[serde(deny_unknown_fields)]` and a passing key-rename tamper test, closing a structural soundness gap that *both* Opus cells had with the same priming. Priming did not just tighten Sonnet's self-score; it surfaced a vulnerability the priming block did not name. The agent reasoned downstream of the threat model the priming block invoked, not just on the specific failure mode cited.

**What the rationale does NOT yet claim.** Haiku is untested with priming in this measurement. The Tools-Haiku-no-SKILLS cell at `canonical-scoring.md` did not self-score (no comparable delta available); the Tools-Haiku-with-SKILLS cell over-claimed by 65 points without priming. The 2026-05-19 measurement does not contain a Haiku-with-priming cell. The plausible read is that priming requires a model with sufficient meta-cognitive capacity to internalise the hostile-judge framing — and that capacity is empirically present in Sonnet 4.6 and Opus 4.7 but unobserved in Haiku 4.7. This is an honest gap, not a refuted claim. Future measurement should run a Haiku-with-priming cell against the same fixture.
...
- **Build-class tasks.** Code generation, evidence-pack emission, MCP wrapper generation, implementation work where the cell's claims can be re-tested against materialised artefacts. The measurement basis is the `pr-evidence-mcp` backbone family; the lift transfers to any task where the rubric is binary-observable.
- **Models with demonstrated meta-cognitive response.** Sonnet 4.6 and Opus 4.7 have observed lifts (`canonical-scoring-sonnet-primed.md` and `canonical-scoring-opus.md`). A new model class SHOULD be measured before being added to the auto-bundle.
...
- **Models without demonstrated meta-cognitive response.** Haiku-class models had no observed lift in Stage 2 (the no-priming Tools-Haiku delta was +65; no Haiku-with-priming cell was measured). Bundling the block for Haiku is *experimental* — the run contract MAY opt in, but the auto-bundle MUST NOT default to it without per-task evidence.
...
The verifier reads the cell's run-contract instance from the audit log, extracts the rendered system prompt, computes its SHA-256 over the priming-block region, and compares to the expected hash. Pass if match; `fail_loud` if not. Reference implementation lives in `tapprove.claim_audit`'s assertion library; see [verifier-packs.md §10](verifier-packs.md#10-cohort-cross-links).

### 5.6 Versioning

The pattern is versioned independently of any consumer. The skill pack's `version` is the load-bearing semver:

- **v1.0.0** — initial; block contents above; hash `c138dd96...`. Bundling required for Sonnet 4.6+ build-class tasks; optional for Opus; experimental for Haiku.
...
- **Calibration tightens at low cost.** Sonnet's canonical delta tightened from +11 to −1 (12-point tightening) for ~200 tokens. The same intervention on Opus produced delta 0 or −2.5. The cost-effectiveness ranking from `canonical-scoring-sonnet-primed.md §5` is: priming >> Tools backbone >> Sonnet→Opus capability upgrade.
- **Downstream surfacing.** Sonnet-primed surfaced `#[serde(deny_unknown_fields)]` as a structural close — a vulnerability the priming block did not name — proving the model reasoned downstream of the threat model the block invoked. Priming generalises beyond the specific failure mode cited.
...
- **~200-token cost per run.** At typical Sonnet 4.6 pricing in mid-2026, the marginal cost is on the order of $0.0005 per cell. At Opus 4.7 pricing, ~$0.002 per cell. The cost is small but not zero; high-frequency build pipelines should track the cumulative cost as part of `performance-and-cost`-class observability ([../principles/performance-and-cost.md](../principles/performance-and-cost.md)).
- **Model-conditional response.** The lift is observed on Sonnet 4.6 and Opus 4.7. There is no measured evidence for Haiku-class models in the Stage 2 backbone; the pattern's `required_for` semver semantics treat Haiku as experimental until measured. Bundling priming for a model with no meta-cognitive response could be net-neutral (200 tokens wasted) or net-harmful (priming may amplify confabulation in models that cannot self-audit).
- **Prompt rot risk.** The block cites the Tools-Haiku silent-stub failure mode by name. If a future failure class displaces silent-stub as the load-bearing exhibit, the citation becomes stale. The pattern's versioning policy (see §5.6) is the mitigation: v1.x.y for additive updates, v2.0.0 for replacement of the cited failure mode.
- **Calibration is not closure.** Priming tightens self-score deltas but does not catch every silent-stub. Tools-Sonnet-primed still missed the post-write re-verify guard (`canonical-scoring-sonnet-primed.md:170-175`) — a 2-point gap. The structural envelope ([run-contracts.md](run-contracts.md) + [verifier-packs.md](verifier-packs.md)) catches what priming misses. Both planes remain mandatory.
- **Per-task auto-bundle policy is opinionated.** The auto-bundle list (`pr-evidence-build`, `mcpact-codegen`, `aegress-implementation`, ...) is doctrine-grade; adding or removing a build-class tag requires a doctrine PR and a verifier-pack-schema review. Operators who want priming on a non-build-class task can still add it explicitly to `context.skills`, but they cannot disable the auto-bundle by skipping the explicit entry.
...
- **Model-conditional response.** Sonnet 4.6 +7, Opus 4.7 0 to −2.5. Haiku 4.7 untested with priming. The pattern's `required_for` field marks Haiku experimental. Future measurement: a 4-cell Haiku-priming sweep (Tools/Native × with/without priming) on the same `pr-evidence-mcp` fixture.
- **Build-class only.** The empirical lift is observed on the `pr-evidence-mcp` backbone family (code generation + evidence emission + MCP wrapping). Pure-reasoning tasks (Q&A, analysis, summarisation without artefacts) are not measured. The pattern does not yet codify whether priming has a measurable lift on those tasks.
- **No multi-run study.** All Stage 2 measurements are single-run per cell. The variance of the lift across N runs is unknown. The +7 Sonnet lift could be ±3 in either direction across a 10-run study; that study has not been done.
- **No counterfactual against other priming styles.** The block above is one of N possible meta-cognitive interventions. Alternative phrasings (e.g. shorter, longer, second-person vs third-person, with vs without the numbered list) are not measured. The current block is the verbatim Opus addendum block adapted to the broader build-class scope; its specific form is not empirically argued against alternatives.
- **Hostile-judge-prediction calibration may flip sign.** Sonnet-primed under-predicted its canonical by 3 points and under-claimed by 1 (`canonical-scoring-sonnet-primed.md:319-331`). Persistent under-claiming has its own costs (deflated cohort signal, missed wins). The pattern does not yet codify a correction for the under-claiming case.
...
| 200 tokens (~231 words) | The empirical block from the Opus addendum is the upper bound of what the measurement validated. Shorter blocks are untested; longer blocks burn budget without observed lift. |
| Required for Sonnet 4.6+, optional for Opus, experimental for Haiku | Empirical: Sonnet shows +7 lift; Opus shows delta 0 to −2.5 (the lift floor); Haiku is unmeasured. The semver semantics reflect the evidence base, not aspirational policy. |
...
- Stage 2 backbone measurement, 5-cell pass (no priming): `value-sheet/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring.md`. Tools-Haiku silent-stub at lines 39-43; Native-Sonnet delta +21 at line 70-ish; full cell comparison table at end of file.
- Stage 2 backbone addendum, Opus pass (priming present): `value-sheet/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring-opus.md`. Tools-Opus delta 0 at line 52; Native-Opus delta −2.5 at line 53.
- Stage 2 backbone addendum II, Sonnet-primed control (apples-to-apples): `value-sheet/18-cross-product-test/v2/results/test-1-backbone/canonical-scoring-sonnet-primed.md`. Lift +7 / delta tightening 12 at lines 238-251; downstream `deny_unknown_fields` surfacing at lines 300-319; under-claim of 3 points at lines 319-331; commercial cost-effectiveness ordering at lines 281-291.
- Operator product directive, 2026-05-19: "priming is a SKILLS-level affordance that lifted Tools-Sonnet canonical from 78 to 85 (delta tightened from −11 to −1). Productize as a canonical doctrine pattern that any operator can import."
