...
The addition path is the existing harness (research → evolution note → ADR → layered guidance → navigation → verification). This section adds the two gates the harness lacks:
...
**Why:** The corpus's promotion precedent (gap note → ADR → file) is real but uncodified; without the two gates, plausible-sounding ideas reach normative standing untested and every gap tends to spawn a file.
