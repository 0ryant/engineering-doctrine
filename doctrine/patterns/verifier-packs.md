...
## 5. Worked Example: `pr-evidence-mcp-verifier-pack`

The operator's verbatim 10-assertion list for the `pr-evidence-mcp` skill, mapped to the v1 schema. The full YAML round-trips through [../../scripts/validate-contracts-v1.py](../../scripts/validate-contracts-v1.py).
...
| 9 | `lockfile_generated` (mcpact.lock) | `lockfile_generated` | `mark_untrusted` | error |
...
```json
{
  "pack_id": "pr-evidence-mcp-verifier-pack",
  "skill": "pr-evidence-mcp",
...
- **`tapprove/crates/tapprove-mcp/src/claim_audit/`** (forthcoming) — canonical pack-execution engine; ships built-in implementations for the 10 standard kinds.
- **`mcpact generate --emit-verifier-pack`** (forthcoming) — generates a starter pack from a skill manifest.
- **Reference adopters:** **corcept** (hook-action observations surface as pack assertions at post-run scope); **aegress** forthcoming (every generated MCP tool ships with a starter pack — silent-stub scaffolds fail their own packs loudly); **cordance-bridge** forthcoming.
...
- **Cross-language assertion sharing.** The 10 kinds have informal semantics here; the Rust impl in `tapprove.claim_audit` is the reference. Ports MUST defer to it for behavioural ties.
